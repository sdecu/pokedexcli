...
func catch(name string) error {
	...
	pokemon, err := parsePokemonJSON(url)
	if err != nil {
		return fmt.Errorf("failed to parse pokemon json:%v", err)
	}

	fmt.Println("Throwing a Pokeball at " + name + "...")

	random := rand.Intn(700) + 1
	if random >= pokemon.BaseExperience {
		pokedex[name] = pokemon
		fmt.Println(name + "caught!")
		return nil
	}
...
func parsePokemonJSON(url string) (pokemonInfo, error) {
